...
// The home page's own composition, in miniature. ActiveSessions is presentational now, so handing it
// a jest.fn() for onDismiss would leave the dismissal never reaching storage and this suite proving
// nothing. The pairing under test is the real hook against the real component, which is exactly what
// src/pages/index.tsx assembles.
const HomePage = (): React.ReactNode => {
  const { entries, onDismiss, onGone } = useJoinedSessions()

  return <ActiveSessions entries={entries} onDismiss={onDismiss} onGone={onGone} />
}
