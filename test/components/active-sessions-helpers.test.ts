import {
  ...
  formatRoster,
  rankCardKind,
} from '@components/active-sessions/helpers'
...
describe('rankCardKind', () => {
  it('leads with the answer, then what is blocking, then what is not', () => {
    const kinds = ['waiting', 'loading', 'winner', 'your-turn'] as const

    expect([...kinds].sort((a, b) => rankCardKind(a) - rankCardKind(b))).toEqual([
      'winner',
      'your-turn',
      'waiting',
      'loading',
  ...
  // The caller breaks ties by joinedAt, so equal kinds have to compare equal rather than merely
  // sort near each other.
  it('ranks two cards in the same state equally', () => {
    expect(rankCardKind('your-turn')).toBe(rankCardKind('your-turn'))
  })
})
