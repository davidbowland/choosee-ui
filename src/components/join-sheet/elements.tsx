...
export const AlreadyJoined = ({
  ...
  <div className="flex flex-col gap-3">
    ...
    {/* Without this the state is a one-way door: mistype into a Choosee you are already in and the
        field is gone, with nothing to do but close the dialog and start again. */}
    <button
...
/**
 * Same shell as `InstallModal` and `QrModal` — blurred backdrop, small container, corner close — so
 * every overlay in the app is the same object wearing different contents.
 *
 * A bottom drawer stretched the field to the full width of a desktop window, which made a two-word
 * code look like a search bar. `placement="auto"` is HeroUI's responsive default: the dialog is
 * pinned to the bottom edge below `sm`, where a thumb is, and centred above it.
 */
export const JoinDialogShell = ({
  children,
  isOpen,
  onClose,
}: {
      ...
      <Modal.Container placement="auto" size="sm">
        <Modal.Dialog>
          ...
          <Modal.Header>
            ...
            <Modal.Heading className="choosee-brand pr-8 text-2xl text-[#F59E0B]">Join a Choosee</Modal.Heading>
          </Modal.Header>
          <Modal.Body>
