    ...
    <Modal.Backdrop variant="blur">
      <Modal.Container size="sm">
        <Modal.Dialog>
          <Modal.CloseTrigger />
          <Modal.Header>
            <Modal.Heading className="pr-8 text-lg text-default-900">Put Choosee on your Home Screen</Modal.Heading>
          </Modal.Header>
          <Modal.Body>
          ...
          <Modal.Footer>
            <Button
              className="w-full rounded-full text-default-600 hover:bg-white/[0.06]"
              onPress={onClose}
              variant="ghost"
            >
              Not now
...
export const InstallOffer = ({ exclusive }: { exclusive: boolean }): React.ReactNode => (
  <div className="flex flex-col gap-1.5">
    <p className="text-sm text-default-700">Opens like an app, straight to a full screen.</p>
...
export const InstallSteps = ({ steps }: { steps: string[] }): React.ReactNode => (
  <ol className="flex flex-col gap-2.5">
    {steps.map((step, index) => (
      <li className="flex items-start gap-2.5 text-sm text-default-800" key={step}>
        <span
          aria-hidden="true"
