 ...
 * themselves — an unnamed voter contributes nothing a reader could recognize.
 *
...
/**
 * What the card wants from you, as a sort key.
 *
 * A winner card is always one you have not opened — markWinnerSeen fires on the winner screen and
 * readJoinedSessions drops the flagged ones — so it is always news, and it is the answer the whole
 * Choosee exists to produce. Your turn comes next: it is the only state blocking other people.
 * Waiting needs nothing from you.
 *
 * `loading` sorts last, and it IS reachable — settling is about the request finishing, not about
 * the card knowing what it is. A session that errors leaves no data to derive a kind from, and a
 * Choosee that is not ready yet has an empty bracket and never enables the voter query, so both
 * settle as `loading` and stay there. Last is the honest place for both: a card that cannot say
 * what it wants from you has no claim on being read first.
 */
export function rankCardKind(kind: CardState['kind']): number {
  switch (kind) {
    case 'winner':
      return 0
    case 'your-turn':
