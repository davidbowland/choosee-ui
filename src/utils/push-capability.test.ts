...
/** A `matchMedia` that answers for exactly one display mode, the way a real browser does. */
const displayMode =
  (mode: string) =>
  (query: string): { matches: boolean } => ({ matches: query === `(display-mode: ${mode})` })
...
describe('push-capability', () => {
  ...
  describe('readCapabilityEnv', () => {
    ...
    it('should detect standalone from the display-mode media query', () => {
      expect(readCapabilityEnv(winOf({ matchMedia: displayMode('standalone') })).isStandalone).toEqual(true)
    })

    it.each(['minimal-ui', 'fullscreen', 'window-controls-overlay'])(
      'should detect an installed app running in %s',
      (mode) => {
        // The manifest asks for standalone; the browser decides what it actually gives us. Firefox
        // for Android runs installed web apps in minimal-ui, and asking only about standalone left
        // the install entry point on screen inside the installed app.
        expect(readCapabilityEnv(winOf({ matchMedia: displayMode(mode) })).isStandalone).toEqual(true)
      },
    )

    it('should not report standalone in an ordinary browser tab', () => {
      expect(readCapabilityEnv(winOf({ matchMedia: displayMode('browser') })).isStandalone).toEqual(false)
    })
